...
- `Get-Force-Liquidation-Record.md` - 获取强平记录（⚠️ 空文件）
...
1. **空文件警告**: `Get-Force-Liquidation-Record.md` 为空
2. **订单限制**: 注意每日订单数量和频率限制
3. **价格精度**: 不同交易对有不同的价格和数量精度要求
4. **杠杆风险**: 杠杆交易风险较高，注意风险控制
5. **小额负债**: 建议定期清理小额负债，避免利息累积
