...
## 请求权重[​](https://developers.binance.com/docs/zh-CN/derivatives/usds-margined-futures/trade/rest-api/New-Algo-Order#%E8%AF%B7%E6%B1%82%E6%9D%83%E9%87%8D "请求权重的直接链接")

IP rate limit(x-mbx-used-weight-1m)为0

## 请求参数[​](https://developers.binance.com/docs/zh-CN/derivatives/usds-margined-futures/trade/rest-api/New-Algo-Order#%E8%AF%B7%E6%B1%82%E5%8F%82%E6%95%B0 "请求参数的直接链接")
...
| timeInForce             | ENUM    | NO   | `IOC` or `GTC` or `FOK` or `GTX` , 默认 `GTC`                                                                   |
...
> - 类型为 `STOP` 的条件订单可以传入参数 `timeInForce`（默认值为 `GTC`）。
> - 类型为 `TAKE_PROFIT` 的条件订单可以传入参数 `timeInForce`（默认值为 `GTC`）。
> 
> <!--THE END-->
...
## 响应示例[​](https://developers.binance.com/docs/zh-CN/derivatives/usds-margined-futures/trade/rest-api/New-Algo-Order#%E5%93%8D%E5%BA%94%E7%A4%BA%E4%BE%8B "响应示例的直接链接")
